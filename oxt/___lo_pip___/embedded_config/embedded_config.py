...
class ConfigMeta(type):
    ...
    def __call__(cls, *args, **kwargs):
        if cls._instance is None:
            ...
            config_file = Path(root, "embedded_config.json")
...
class EmbeddedConfig(metaclass=ConfigMeta):
    """
    ...
    def __init__(self, **kwargs) -> None:
        """Constructor"""
        self._32_bit = cast(Dict[str, EmbeddedItemT], kwargs["32_bit"])
        self._64_bit = cast(Dict[str, EmbeddedItemT], kwargs["64_bit"])
        self._config = Config()
    ...
    def get_config(self) -> EmbeddedItemT:
        """Gets the bz2 config for the current python version and Operating System."""
        if self._is_32_bit:
            return cast(EmbeddedItemT, self._32_bit[self._config.python_major_minor])
        else:
            return cast(EmbeddedItemT, self._64_bit[self._config.python_major_minor])
